# HIP-Doc Linker APIs Options Example

## Description

This example demonstrates how to link at runtime a LLVM bitcode object (which is stored in memory) using the HIPRTC API.
During the link stage an array of linker options is passed to the runtime linker. For more information on this topic,
please refer to the
[HIP documentation](https://rocm.docs.amd.com/projects/HIP/en/latest/how-to/hip_rtc.html#link-options).
...
7. A HIPRTC link instance is created. During creation linker flags are passed to the HIPRTC API.
...
* `hiprtcLinkCreate` creates a HIPRTC link instance. Optionally, linker flags can be passed with this API call.
