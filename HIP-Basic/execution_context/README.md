...
2. The total number of compute units is queried with `hipDeviceGetDevResource` using the `hipDevResourceTypeSm` resource type. (The field is named `smCount` for CUDA source compatibility; on AMD GPUs it represents compute units.)
3. The CU resources are split into two groups with `hipDevSmResourceSplit`: a larger group for the long-running kernel and a smaller group reserved for the critical kernel.
...
6. A stream is created for each execution context with `hipExecutionCtxStreamCreate`.
7. A busy kernel is launched and timed on each stream using HIP events. The long-running kernel oversubscribes its CUs; the critical kernel runs a shorter workload on its reserved CUs.
8. The device output buffers are freed with `hipFree`.
9. The streams are destroyed with `hipStreamDestroy`.
10. The execution contexts are destroyed with `hipExecutionCtxDestroy`.
