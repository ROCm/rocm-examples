...
- GLFW3 is required, which can either be installed through the package manager or obtained from https://www.glfw.org/. If using CMake, the `glfw3Config.cmake` file must be in a path that CMake searches by default or must be passed using `-DCMAKE_MODULE_PATH`. The official GLFW3 binaries do not ship this file on Windows, and so GLFW3 must be compiled manually. CMake will be able to find GLFW3 on Windows if it is installed installed in `C:\Program Files (x86)\glfw3`. If using Visual Studio, the inappropriate path to the GLFW3 install directory should be set in Properties->Linker->General->Additional Library Directories and Properties->C/C++->General->Additional Include Directories.
