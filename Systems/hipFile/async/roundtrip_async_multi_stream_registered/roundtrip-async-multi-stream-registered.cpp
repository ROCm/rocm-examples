 ...
 *
 * Usage: ./roundtrip_async_multi_stream_registered READ_FILE WRITE_FILE [GPUID]
 *
 ...
 *   4. Open READ_FILE (O_RDONLY|O_DIRECT) and WRITE_FILE
 ...
 *        hipFileStreamDeregister -> hipStreamDestroy -> hipFileBufDeregister ->
 * hipFree
 */
...
static_assert((SLICE_SIZE % BLOCK_ALIGN) == 0,
              "SLICE_SIZE must be a multiple of BLOCK_ALIGN");
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < NUM_STREAMS; ++i) {
    ...
    if (hipSuccess != hip_err) {
      fprintf(stderr,
              "Could not allocate %zu bytes on GPU %d for slice %d (%d)\n",
  ...
  for (int i = 0; i < NUM_STREAMS; ++i) {
    slice_state &s = slices[i];
    hipfile_err =
        hipFileReadAsync(rhandle, s.devbuf, &s.io_size, &s.file_offset,
  ...
  for (int i = 0; i < NUM_STREAMS; ++i) {
    slice_state &s = slices[i];
    hipfile_err =
        hipFileWriteAsync(whandle, s.devbuf, &s.io_size, &s.file_offset,
...

  {
    ...
    printf("OK  %s == %s  (%zu bytes across %d registered streams, hash "
           "0x%016" PRIx64 ")\n",
...
cleanup_slices:
  /* Teardown order:
   *   hipFileStreamDeregister -> hipStreamDestroy -> hipFileBufDeregister ->
   * hipFree Per-entry bool flags handle partial setup from a failed init loop.
   */
  for (int i = 0; i < NUM_STREAMS; ++i) {
    slice_state &s = slices[i];
