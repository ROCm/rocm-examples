...
int main(int argc, char *argv[]) {
   ...
   * deposited without an intermediate sync. */
  {
    ...
    hipfile_err = hipFileReadAsync(rhandle, devbuf, &rsize, &rfile_offset,
                                   &rbuf_offset, &bytes_read, stream);
    if (hipFileSuccess != hipfile_err.err) {
    ...
    hipfile_err = hipFileWriteAsync(whandle, devbuf, &wsize, &wfile_offset,
                                    &wbuf_offset, &bytes_written, stream);
    if (hipFileSuccess != hipfile_err.err) {
