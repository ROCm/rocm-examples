...
This program demonstrates asynchronous GPU-direct I/O using hipFile on an explicit
...
default stream, enabling finer-grained concurrency control.
...
- `hipStreamCreateWithFlags(hipStreamNonBlocking)` creates a stream that does not synchronize
  with the NULL (legacy default) stream. This is the preferred choice for async hipFile I/O in
  applications that also issue work on the default stream, since it prevents unintended
  serialization.
- `hipStreamNonBlocking` streams still provide intra-stream ordering guarantees: operations
  submitted to the same stream execute in submission order.
