 ...
 *            of INPUT starting at SW_SUB_OFFSET (size = payload -
 * SW_SUB_OFFSET). GPUID    GPU device index (optional, default 0).
 *
 * Steps:
 *   1. Select GPU
...
int main(int argc, char *argv[]) {
  ...
  if (sub_offset >= payload_size) {
    fprintf(stderr,
            "SW_SUB_OFFSET (%zu) must be smaller than the consumed payload "
            "(%zu bytes)\n",
  ...
  nbytes = hipFileRead(in_handle, devbuf, alloc_size, /*file_offset=*/0,
                       /*buf_offset=*/0);
  if (nbytes < 0) {
...

  {
    ...
    printf("OK  %s[%zu:%zu] -> %s  (%zu bytes, hash 0x%016" PRIx64 ")\n",
           in_path, sub_offset, payload_size, out_path, write_size,
           hash_in_tail);
  }
