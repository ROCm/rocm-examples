 ...
 *              2 = managed memory     (hipMallocManaged - no BufRegister
 * needed) 3 = pinned host memory (hipHostMalloc) GPUID    GPU device index
 * (optional, default 0).
 *
 * Steps:
 *   1. Select GPU
...
static int alloc_buf(mem_mode_t mode, size_t size, void **out_buf) {
  hipError_t hip_err;
  switch (mode) {
  case MEM_DEVICE:
...
static int free_buf(mem_mode_t mode, void *buf) {
  hipError_t hip_err;
  switch (mode) {
  case MEM_DEVICE:
  case MEM_MANAGED:
...
static int zero_buf(mem_mode_t mode, void *buf, size_t size) {
  if (mode == MEM_PINNED) {
    memset(buf, 0, size);
    return 0;
  }
...
int main(int argc, char *argv[]) {
  ...
  if (mode_raw < 1 || mode_raw > 3) {
    fprintf(stderr,
            "MODE must be 1 (device), 2 (managed), or 3 (pinned-host)\n");
    return EXIT_FAILURE;
  }
  ...
  nbytes = hipFileRead(in_handle, buf, alloc_size, /*file_offset=*/0,
                       /*buf_offset=*/0);
  if (nbytes < 0) {
  ...
  /* 5. Open + register output file + hipFileWrite + ftruncate */
  if (open_file(out_path, O_WRONLY | O_CREAT | O_TRUNC | O_DIRECT,
                S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH, &out_fd, &out_handle)) {
    goto release_buf;
  }

  nbytes = hipFileWrite(out_handle, buf, alloc_size, /*file_offset=*/0,
                        /*buf_offset=*/0);
  if (nbytes < 0) {
