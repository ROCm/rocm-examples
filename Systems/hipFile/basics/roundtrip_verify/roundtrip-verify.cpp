...
int main(int argc, char *argv[]) {
  ...
  nbytes = hipFileWrite(handle, devbuf, alloc_size, /*file_offset=*/0,
                        /*buf_offset=*/0);
  if (nbytes < 0) {
  ...
  nbytes = hipFileRead(handle, devbuf, alloc_size, /*file_offset=*/0,
                       /*buf_offset=*/0);
  if (nbytes < 0) {
  ...
  if (open_file(copied_path, O_WRONLY | O_CREAT | O_TRUNC | O_DIRECT,
                S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH, &fd, &handle)) {
    goto deregister_buf;
  }
