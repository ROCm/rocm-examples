...
static int open_file(const char *path, int flags, mode_t mode, int *fd,
...
/// @return zero on success, non-zero on failure
static int close_file(const char *path, int fd, hipFileHandle_t handle) {
...
/// @brief Round value to the next multiple of align. Align _must_ be a power
/// of 2.
...
int main(int argc, char *argv[]) {
  ...
  if (open_file(dst_path, O_WRONLY | O_CREAT,
                S_IWUSR | S_IRUSR | S_IRGRP | S_IROTH, &dst_fd, &dst_handle)) {
    goto program_exit;
  }
    ...
    while (nwrite < nread) {
      ...
      if (nbytes < 0) {
        fprintf(stderr, "Could not write to %s (%zd) (%s)\n", dst_path, nbytes,
                IS_HIPFILE_ERR(nbytes) ? HIPFILE_ERRSTR(nbytes)
                                       : strerror(errno));
        goto free_devbuf;
      }
      nwrite += nbytes;
