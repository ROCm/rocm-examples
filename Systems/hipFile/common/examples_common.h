...
/// @brief Round value up to the next multiple of align. Align _must_ be a power
/// of 2.
...
/// @brief Read size bytes of path starting at byte offset, then FNV-1a hash
/// them. Reads via plain POSIX I/O (no O_DIRECT), so this is the host-side
/// reference path for verifying what hipFile wrote out.
...
/// @param out_hash [out] Receives the FNV-1a hash on success; untouched on
/// failure.
/// @return zero on success, non-zero on failure.
int hash_file_range(const char *path, off_t offset, size_t size,
                    uint64_t *out_hash);
...
int verify_files_match(const char *read_path, const char *write_path,
                       size_t size, uint64_t *out_hash);
...
/// @param flags  [in]  Flags to pass to open(2). O_DIRECT is _not_ added for
/// you.
...
/// @return zero on success, non-zero on failure.
int open_file(const char *path, int flags, mode_t mode, int *fd,
