# hipBLASLt Extension API - GEMM with Absolute Maximum

## Description

This example illustrates the use of the `hipBLASLt` extension API for general matrix multiplication with integrated absolute maximum computation.
...
7. Allocate device memory for AMAX output value.
8. Set up GEMM epilogue (default operation).
9. Configure GEMM inputs including matrices and AMAX output pointer.
...
13. Execute the GEMM operation with integrated AMAX computation.
14. Copy AMAX result from device to host and display the value.
...
- **GEMM Configuration**:
  ...
  - `hipblaslt_ext::GemmInputs`: Specifies the input matrices and scalars for the GEMM operation. `setAmaxD()` is used to enable AMAX computation on the output matrix.
