# hipBLASLt GEMM with Swish Activation and Bias Addition

## Description

This example illustrates the use of the `hipBLASLt` library for general matrix multiplication with Swish activation and bias addition.

The operation calculates the following with Swish activation and bias addition:

$D = \text{Swish}(\alpha \cdot op_A(A) \cdot op_B(B) + \beta \cdot C + bias)$
...
- $\text{Swish}(x) = x \cdot \sigma(x)$ where $\sigma(x) = \frac{1}{1 + e^{-x}}$ is the sigmoid function
...
8. Configure SWISH_BIAS_EXT epilogue for combined Swish activation and bias addition.
9. Allocate and initialize bias vector with example values.
10. Configure bias tensor with data type and pointer.
...
13. Perform matrix multiplication with integrated Swish activation and bias addition.
...
- **GEMM Operation Descriptor**:
  ...
  - `hipblasLtMatmulDescSetAttribute()`: Sets details of the GEMM operation. For the Swish activation with bias, the epilogue is set to `HIPBLASLT_EPILOGUE_SWISH_BIAS_EXT`, and the bias pointer and data type are configured using `HIPBLASLT_MATMUL_DESC_BIAS_POINTER` and `HIPBLASLT_MATMUL_DESC_BIAS_DATA_TYPE`.
...
- **Execution**:
  - `hipblasLtMatmul()`: Executes the matrix multiplication with the Swish activation and bias addition using a selected algorithm from the heuristic results.
...
- **Key Enumerations**:
  ...
  - `hipblasLtEpilogue_t`: Defines the epilogue operation, such as `HIPBLASLT_EPILOGUE_SWISH_BIAS_EXT`.
