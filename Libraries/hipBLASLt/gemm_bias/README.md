# hipBLASLt GEMM with Bias Addition

## Description

This example illustrates the use of the `hipBLASLt` library for general matrix multiplication with bias addition.
...
8. Configure BIAS epilogue for bias addition operation.
9. Allocate and initialize bias vector with example values.
10. Set bias data type and pointer in the matrix multiplication descriptor.
...
13. Perform matrix multiplication with integrated bias addition.
...
- **GEMM Operation Descriptor**:
  ...
  - `hipblasLtMatmulDescSetAttribute()`: Sets details of the GEMM operation. For bias addition, the epilogue is set to `HIPBLASLT_EPILOGUE_BIAS`, and the bias pointer and data type are configured using `HIPBLASLT_MATMUL_DESC_BIAS_POINTER` and `HIPBLASLT_MATMUL_DESC_BIAS_DATA_TYPE`.
...
- **Execution**:
  - `hipblasLtMatmul()`: Executes the matrix multiplication with bias addition using a selected algorithm from the heuristic results.
