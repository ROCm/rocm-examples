# hipBLASLt GEMM with Matrix B Swizzling

## Description

This example illustrates the use of the `hipBLASLt` library for general matrix multiplication with matrix B swizzling optimization.

The operation calculates the following matrix product with swizzled memory layout:

$D = \alpha \cdot op_A(A) \cdot op_B(B_{swizzled}) + \beta \cdot C$

where

- $\alpha$ and $\beta$ are scalars
- $B_{swizzled}$ is matrix B with optimized memory layout (swizzled pattern)
...
2. Execute two different GEMM configurations for comparison:
   a. **Regular GEMM**: Standard matrix multiplication without swizzling (FP16)
   b. **Swizzled GEMM**: Matrix B swizzling with FP16 data types
3. For each configuration:
   ...
   c. Apply swizzling transformation to matrix B when enabled
   d. Execute matrix multiplication
...
- **Matrix Descriptors**:
  ...
  - `hipblasLtMatrixLayoutSetAttribute()`: Configures matrix properties. For swizzling, `HIPBLASLT_MATRIX_LAYOUT_ORDER` is set to `HIPBLASLT_ORDER_COL16_4R8` for FP16 data types.
...
- **Swizzling Transformation**:
  - Custom `swizzle_tensor()` function: Applies the swizzling pattern to matrix B by reshaping and permuting tensor dimensions for optimized memory access patterns.
  - The transformation reorganizes data to improve memory coalescing and computational efficiency on GPU hardware.
