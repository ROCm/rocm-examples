# hipBLASLt GEMM with Absolute Maximum

## Description

This example illustrates the use of the `hipBLASLt` library for general matrix multiplication with integrated absolute maximum computation (see: `hipblasLtMatmul()` with AMAX attributes in [Key APIs and Concepts/hipBLASLt](#hipblaslt)).

The operation calculates the following product with additional AMAX computation:
...
- $AMAX$ is the absolute maximum value of all elements in the output matrix $D$
- $op_A(A)$ and $op_B(B)$ are the result of applying to matrices $A$ and $B$, respectively, one of the `hipblasOperation_t` operations described below in [Key APIs and Concepts - hipBLASLt](#hipblaslt).
...
3. Allocate additional memory for AMAX output and scaling infrastructure.
4. Initialize scaling factor (1.0 - no effective scaling) and copy to device memory.
...
9. Configure AMAX attribute in the matrix multiplication descriptor (scaling is set to 1.0 for no effect).
...
12. Perform matrix multiplication with integrated AMAX computation.
...
- **GEMM Operation Descriptor**:
  ...
  - `hipblasLtMatmulDescSetAttribute()`: Sets details of the GEMM operation. For AMAX computation, `HIPBLASLT_MATMUL_DESC_AMAX_D_POINTER` is used to specify the output pointer for the absolute maximum value.
...
- **Execution**:
  - `hipblasLtMatmul()`: Executes the matrix multiplication, including the AMAX computation, using a selected algorithm from the heuristic results.
