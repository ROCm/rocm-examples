# hipBLASLt Extension API - GEMM with Workgroup Mapping Tuning

## Description

This example illustrates the use of the `hipBLASLt` extension API for general matrix multiplication with Workgroup Mapping (WGM) tuning optimization.

The operation calculates the following matrix product with WGM optimization:
...
- Workgroup mapping is optimized for improved GPU resource utilization
...
10. Create tuning configurations: default and WGM with factor 4.
11. Validate algorithm support for both tuning configurations.
12. Allocate additional workspace if WGM requires more memory.
13. Execute GEMM with default tuning configuration.
14. Re-initialize and execute GEMM with WGM tuning configuration.
...
- **GEMM Configuration**:
  ...
  - `hipblaslt_ext::GemmTuning`: Configures tuning parameters, such as the Workgroup Mapping (WGM) factor with `setWgm()`.
...
- `hipblaslt_ext::GemmTuning::setWgm`
