# rocProfiler SDK Buffer Counter Collection

## Description

This example demonstrates how to use the rocProfiler SDK to collect hardware counters from kernel dispatches using a buffer-based approach. It sets up a profiling context, configures a buffer to store counter data, and registers a callback that is invoked when the buffer is full. The collected data is then processed and printed to the console.
...
6. **Data Collection and Processing**:
    ...
    - This callback processes the records, filters for counter data, and prints the results, including the dispatch ID, kernel ID, and counter value.
7. **Finalization**:
...
- **Buffer-Based Counter Collection**:
  ...
  - `rocprofiler_create_buffer()`: Creates a buffer to store the counter records. This example uses a lossless policy (`ROCPROFILER_BUFFER_POLICY_LOSSLESS`) to ensure no data is dropped.
...
- **Counter Configuration**:
  ...
  - `rocprofiler_query_counter_info()`: Retrieves detailed information about a specific counter.

- **Agent and Record Querying**:
  - `rocprofiler_query_available_agents()`: Used to find all available agents, which are then filtered for GPU agents.
  - `rocprofiler_query_record_counter_id()` and `rocprofiler_query_record_dimension_position()`: Used in the buffer callback to extract the counter ID and dimension information from the records.
