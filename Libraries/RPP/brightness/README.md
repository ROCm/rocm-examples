...
This example illustrates the use of the `RPP` library for adjusting image brightness. The brightness operation applies a linear transformation to pixel values using the formula:

$output = \alpha \cdot input + \beta$

where $\alpha$ is a multiplier (contrast factor) and $\beta$ is an offset (brightness adjustment).

## Application flow

1. Parse command-line arguments for input/output paths, bit depth, layout type, alpha, and beta values.
...
11. Set up alpha and beta parameter tensors for batch processing.
12. Execute brightness adjustment operation on GPU.
