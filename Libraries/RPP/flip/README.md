...
This example illustrates the use of the `RPP` library for flipping images. The flip operation can mirror images horizontally, vertically, or both, creating reflections along the specified axes.

The operation supports three flip modes:

- Horizontal flip: Mirror along the vertical axis (left-right reflection)
- Vertical flip: Mirror along the horizontal axis (top-bottom reflection)
- Both: Mirror along both axes (180-degree rotation)

## Application flow

1. Parse command-line arguments for input/output paths, bit depth, layout type, and flip axis.
2. Load image names from the input folder and validate flip axis parameter.
...
11. Set up horizontal and vertical flip parameter tensors for batch processing.
12. Execute flip operation on GPU with specified flip axes.
...
- **Flip Operation**:
  ...
  - Each image in the batch can have different flip settings.
