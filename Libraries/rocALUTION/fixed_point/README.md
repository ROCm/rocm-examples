# rocALUTION Fixed-Point Iteration

## Description

This example demonstrates the use of the `rocALUTION` library for solving linear systems using the Fixed-Point iteration method.
...
The Fixed-Point iteration method reformulates the linear system as $x = x + \omega (b - Ax)$ and iteratively updates the solution. The relaxation parameter $\omega$ controls the convergence rate and stability of the iteration. This method is particularly useful as a smoothing technique in multigrid methods and as a simple iterative solver.
...
10. Configure the Fixed-Point solver with Jacobi preconditioner.
11. Set relaxation parameter $\omega$ to 2/3 for optimal convergence.
...
- **Fixed-Point-Specific Configuration**:
  - `rocalution::FixedPoint::SetRelaxation()`: Sets the relaxation parameter $\omega$ for the fixed-point iteration scheme.
