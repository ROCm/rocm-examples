# rocALUTION CG with Smoothed Aggregation AMG Preconditioner
...
This example shows how to combine the CG solver with Smoothed Aggregation AMG preconditioning for optimal performance on large symmetric positive definite systems. The SAAMG uses PMIS coarsening strategy, AddWeakConnections lumping strategy, and specific coupling strength parameters for robust multigrid hierarchy construction.
...
9. Configure the CG solver with Smoothed Aggregation AMG preconditioner:
   - Set coarsening strategy to PMIS (Parallel Modified Independent Set)
   - Set lumping strategy to AddWeakConnections
...
- **Smoothed Aggregation AMG Configuration**:
  - `rocalution::SAAMG::SetCoarseningStrategy()`: Sets the coarsening strategy (PMIS).
  - `rocalution::SAAMG::SetLumpingStrategy()`: Sets the lumping strategy (AddWeakConnections).
...
- **Smoothed Aggregation AMG**:
  - **Aggregation-Based Coarsening**: Groups strongly connected nodes into aggregates.
  - **PMIS Strategy**: Parallel Modified Independent Set for optimal parallel performance.
  - **Weak Connection Lumping**: Adds weak connections to improve aggregation quality.
  - **Smoothing**: Applies smoothing to interpolation operators for better convergence.
...
- `rocalution::SAAMG::SetLumpingStrategy`
