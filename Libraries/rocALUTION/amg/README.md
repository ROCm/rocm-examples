# rocALUTION Smoothed Aggregation Algebraic Multigrid (SAAMG)

## Description

This example demonstrates the use of the `rocALUTION` library for solving linear systems using Smoothed Aggregation Algebraic Multigrid method.
...
The Smoothed Aggregation Algebraic Multigrid method is a multigrid algorithm that automatically constructs a hierarchy of coarser grids to accelerate convergence. It's particularly effective for large sparse systems arising from partial differential equations.
...
9. Configure the SAAMG solver with specific parameters:
   - Set coupling strength to 0.001
   - Set coarsest level to 300 unknowns
   - Set interpolation relaxation to 2/3
   - Enable manual smoothers and solver
   - Enable grid transfer scaling
10. Build the AMG hierarchy and obtain the number of levels.
11. Create and configure coarse grid solver (CG) and smoothers for each level.
12. Set up Fixed-Point iteration scheme with Multi-Colored Gauss-Seidel preconditioner.
13. Configure smoother pre and post iteration counts.
14. Initialize solver tolerances and build the complete solver.
15. Move all objects to the accelerator (GPU).
16. Print matrix information and measure build time.
17. Solve the linear system $Ax = b$ and measure solve time.
18. Clear solver resources and free allocated smoother objects.
19. Compute and report the L2 norm of the error.
20. Stop the rocALUTION platform.
