...
RunState __device__ malloc_run_state(Config *p) {
  ...
  s.key_cache =
      hip::std::make_unique<float[]>(p->n_layers * p->seq_len * kv_dim);
  s.value_cache =
      hip::std::make_unique<float[]>(p->n_layers * p->seq_len * kv_dim);
...
TransformerWeights __device__ memory_map_weights(Config *p, float *ptr,
                                                 int shared_weights) {
...
Checkpoint read_checkpoint(const char *checkpoint_path) {
  ...
  long long file_size =
      ftell(file); // win.h redefines ftell to _ftelli64 on Windows
  fclose(file);
  ...
  thrust::device_ptr<float> weights_ptr_dev =
      thrust::device_malloc<float>(num_floats);
...
std::tuple<Transformer *, Config, float *>
build_transformer(const char *checkpoint_path) {
  // read in the Config and the Weights from the checkpoint
  auto [config, weights_ptr, shared_weights] = read_checkpoint(checkpoint_path);

  // Allocate device-resident Transformer
  thrust::device_ptr<Transformer> transformer_ptr_dev =
      thrust::device_malloc<Transformer>(1);
  ...
  hip::wthread(
      [] __device__(Transformer * t, float *weights_ptr, int shared_weights) {
        ...
        t->weights =
            memory_map_weights(&t->config, weights_ptr, shared_weights);

        // Initialize RunState
        // Use placement-new so we don't try to assign to unique_ptrs that think
        // they already point at something
...
void free_transformer(Transformer *transformer, float *weights_ptr) {
  ...
  hip::wthread([] __device__(Transformer * t) { t->state.~RunState(); },
               transformer)
      .join();
...
constexpr int __host__ __device__ get_chunk_size(int arr_size, int tid,
                                                 int num_threads) {
  return (tid < arr_size % num_threads) ? (arr_size / num_threads + 1)
                                        : (arr_size / num_threads);
}

constexpr int __host__ __device__ get_start_row(int arr_size, int tid,
                                                int num_threads) {
  int chunk_size = get_chunk_size(arr_size, tid, num_threads);
  return (tid < arr_size % num_threads)
             ? (tid * chunk_size)
             : (tid * chunk_size + arr_size % num_threads);
}

// Spin-lock barrier for n threads. barrier must be big enough to count
// 3*nThreads - 1 without overflowing.
template <uint32_t nThreads> uint32_t __device__ barrier_wait() {
...
void __device__ matmul(float *xout, float *x, float *w, int n, int d, int tid,
                       int num_threads) {
  // W (d,n) @ x (n,) -> xout (d,)
  ...
  for (int i = start_row + hip::this_thread::get_fiber_id();
       i < start_row + chunk_size; i += hip::this_thread::get_width()) {
    float val = 0.0f;
    for (int j = 0; j < n; ++j) {
      val += w[i * n + j] * x[j];
    }
...
std::unique_ptr<float[]> forward(Transformer *transformer, int token, int pos,
                                 int vocab_size) {
  ...
  for (int tid = 0; tid < num_threads; ++tid) {
    threads[tid] = hip::wthread(
        ...
        [] __device__(Transformer * t, int token, int pos, int tid) {
          ...
          const int kv_mul =
              p->n_heads / p->n_kv_heads; // integer multiplier of the kv
                                          // sharing in multiquery
          ...
          for (unsigned long long l = 0; l < p->n_layers; l++) {
            const int loff = l * p->seq_len *
                             kv_dim; // kv cache layer offset for convenience
            // Regardless of whether we are entering the loop for the first time
            // or we just finished looping, all the work since the last
            // barrier_wait has been within an `if (tid == 0)`. Thus, no
            // barrier_wait is needed and a simple __syncthreads() will be
            // sufficient. attention rmsnorm
            if (tid == 0) {
              __syncthreads();
            ...
            for (int i = tid; i < p->n_heads; i += num_threads) {
              for (int j = hip::this_thread::get_fiber_id() * 2; j < head_size;
                   j += 2 * hip::this_thread::get_width()) {
                float freq =
                    1.0f / powf(500000.0f, (float)j / (float)head_size);
                float val = pos * freq;
                float fcr = cosf(val);
                float fci = sinf(val);
            ...
            for (int h = tid; h < p->n_heads; h += num_threads) {
              ...
              for (int t = hip::this_thread::get_fiber_id(); t <= pos;
                   t += hip::this_thread::get_width()) {
                // get the key vector for this head and at this timestep
                float *k = s->key_cache.get() + loff + t * kv_dim +
                           (h / kv_mul) * head_size;
              ...
              // Sync all the fibers after the above loop, but not across
              // threads
              __syncthreads();
              ...
              // inclusively
              softmax(att, pos + 1);
              // Sync all the fibers after the above softmax call, but not
              // across threads
              __syncthreads();

              // weighted sum of the values, store back into xb
              float *xb = s->xb.get() + h * head_size;
              for (int i = hip::this_thread::get_fiber_id(); i < head_size;
            ...
            if (tid == 0) {
              for (int i = hip::this_thread::get_fiber_id(); i < dim;
                   i += hip::this_thread::get_width()) {
                x[i] += s->xb2[i];
              }
              __syncthreads();
            ...
            matmul(s->hb.get(), s->xb.get(), w->w1 + l * dim * hidden_dim, dim,
                   hidden_dim, tid, num_threads);
            matmul(s->hb2.get(), s->xb.get(), w->w3 + l * dim * hidden_dim, dim,
                   hidden_dim, tid, num_threads);
            ...
            // No barrier_wait is needed because the preceding two matmul calls
            // use the same chunking as we will for SwiGLU, thus each fiber is
            // only reading from array entries it wrote to. A simple
            // __syncthreads() will therefore be sufficient.
            __syncthreads();
            ...
            matmul(s->xb.get(), s->hb.get(), w->w2 + l * dim * hidden_dim,
                   hidden_dim, dim, tid, num_threads);
            barrier_wait<num_threads>();
          ...
          // final rmsnorm
          // All the work since the last barrier_wait (in the for loop) was
          // within an `if (tid == 0)`. Thus, no barrier_wait is needed and a
          // simple __syncthreads() will be sufficient.
          if (tid == 0) {
            __syncthreads();
          ...
          matmul(s->logits.get(), x, w->wcls, p->dim, p->vocab_size, tid,
                 num_threads);
        },
  ...
  // Copy logits back to host. We can't use thrust APIs here because there is
  // still a hip::wthread on the stack from generate/chat.
  float *logits_device_ptr;
  HIP_CHECK(hipMemcpyAsync(&logits_device_ptr, &transformer->state.logits_raw,
                           sizeof(float *), hipMemcpyDeviceToHost, g_stream));
  HIP_CHECK(hipStreamSynchronize(g_stream));
  HIP_CHECK(hipMemcpyAsync(logits.get(), logits_device_ptr,
...
int str_lookup(char *str, TokenIndex *sorted_vocab, int vocab_size) {
  ...
  TokenIndex *res = reinterpret_cast<TokenIndex *>(bsearch(
...
void encode(Tokenizer *t, const char *text, int8_t bos, int8_t eos, int *tokens,
  ...
  if (t->sorted_vocab == NULL) {
    ...
    t->sorted_vocab = reinterpret_cast<TokenIndex *>(
        malloc(t->vocab_size * sizeof(TokenIndex)));
    for (int i = 0; i < t->vocab_size; i++) {
  ...
  char *str_buffer = reinterpret_cast<char *>(
...
void generate(Transformer *transformer, Tokenizer *tokenizer, Sampler *sampler,
  ...
  // Constructing a dummy hip::wthread keeps the persistent scheduler kernel
  // alive across iterations of the while loop. This prevents us from repeatedly
  // spinning up/down every time we call forward
  hip::wthread _;
  ...
  while (pos < steps) {
    ...
    std::unique_ptr<float[]> logits =
        forward(transformer, token, pos, sampler->vocab_size);
...
void error_usage() {
  ...
  fprintf(stderr, "  -n <int>    number of steps to run for, default 4096. 0 = "
                  "max_seq_len\n");
...
int main(int argc, char *argv[]) {
  ...
  // build the Transformer via the model .bin file (returns device pointer,
  // config, and weights pointer)
