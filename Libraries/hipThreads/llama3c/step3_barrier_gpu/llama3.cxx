...
Checkpoint read_checkpoint(const char *checkpoint_path) {
  ...
  long long file_size =
      ftell(file); // win.h redefines ftell to _ftelli64 on Windows; long is
                   // 32-bit on Windows so must use long long for >2GB files
  fclose(file);
    ...
    threads[tid] = hip::wthread(
        [] __device__(Transformer * t, int token, int pos, int tid) {
          ...
          for (unsigned long long l = 0; l < p->n_layers; l++) {
            ...
            // use the same chunking as we will for SwiGLU, thus each thread is
            // only reading from array entries it wrote to. SwiGLU non-linearity
            for (int i = start_row; i < start_row + chunk_size; i++) {
              float val = s->hb[i];
