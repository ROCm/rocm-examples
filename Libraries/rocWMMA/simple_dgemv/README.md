# rocWMMA Simple Double-Precision General Matrix-Vector Multiplication (DGEMV)

## Description

This example demonstrates double-precision General Matrix-Vector multiplication (GEMV) using rocWMMA. It shows how to adapt WMMA operations for matrix-vector computations with FP64 precision.
...
1. **Device Capability Check**: The application checks if the device supports double-precision (FP64) operations.
...
4. **Kernel Launch**: The `dgemv_rocwmma_d` kernel is launched. The grid and block dimensions are configured for the matrix-vector operation.
