# rocWMMA Performance Double-Precision General Matrix Multiplication (DGEMM)

## Description

This example showcases an optimized double-precision GEMM implementation with advanced cooperative loading and data reuse techniques for maximum FP64 performance. It demonstrates how to achieve high computational throughput for scientific computing applications requiring double-precision accuracy.
...
1. **Device Capability Check**: The application first checks if the device supports double-precision (FP64) operations.
...
5. **Kernel Execution**: The `gemm_rocwmma_d` kernel is launched with the configured parameters. The kernel performs the following steps:
