...
bool run(const ck_tile::ArgParser& arg_parser)
{
    ...
    std::printf("N: %d, D: %d, H: %d, W: %d, C: %d\n", N, D, H, W, C);
    std::printf("Window Z: %d, Y: %d, X: %d, Stride Z: %d, Y: %d, X: %d\n", Z, Y, X, Sz, Sy, Sx);
    std::printf("Output Do: %d, Ho: %d, Wo: %d\n", Do, Ho, Wo);
...
int main(int argc, char* argv[])
{
    ...
    return run<ck_tile::half_t, ck_tile::half_t, float>(arg_parser) ? EXIT_SUCCESS : EXIT_FAILURE;
}
