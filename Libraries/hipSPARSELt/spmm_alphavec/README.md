# hipSPARSELt Sparse Matrix-Matrix Multiplication with Alpha Vector Example
...
$D = \alpha^{\text{T}} \times \mathbf{A} \times \mathbf{B}^{\text{T}} + \beta \times \mathbf{C}$, where $\alpha$ is a
vector, $\beta$ is a scalar value, $\mathbf{A}$ is a sparse matrix and $\mathbf{B}$, $\mathbf{C}$ and $\mathbf{D}$ are
dense matrices.
...
8. The matrix multiplication is modified by setting an attribute which specifies that $\alpha$ is a vector.
