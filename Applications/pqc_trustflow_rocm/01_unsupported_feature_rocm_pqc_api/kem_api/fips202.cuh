/*
 * Aigis-enc GPU - FIPS 202 (SHA3/SHAKE) 设备函数
 * 与算法无关，Kyber 和 Aigis-enc 共用同一实现
 */
#ifndef FIPS202_CUH
#define FIPS202_CUH
...
{
    for (unsigned int i = 0; i < 8; ++i) { x[i] = (uint8_t)u; u >>= 8; }
}
...
static __device__ __noinline__ void KeccakF1600_StatePermute(uint64_t *state)
{
    ...
    for (round = 0; round < NROUNDS; round += 2) {
        ...
        Di = BCe^ROL(BCo, 1); Do = BCi^ROL(BCu, 1); Du = BCo^ROL(BCa, 1);

        Aba ^= Da; BCa = Aba;
        Age ^= De; BCe = ROL(Age, 44); Aki ^= Di; BCi = ROL(Aki, 43);
        Amo ^= Do; BCo = ROL(Amo, 21); Asu ^= Du; BCu = ROL(Asu, 14);
        Eba = BCa ^((~BCe)& BCi); Eba ^= gpu_KeccakF_RoundConstants[round];
        ...
        Eba ^= Da; BCa = Eba;
        Ege ^= De; BCe = ROL(Ege, 44); Eki ^= Di; BCi = ROL(Eki, 43);
        Emo ^= Do; BCo = ROL(Emo, 21); Esu ^= Du; BCu = ROL(Esu, 14);
        Aba = BCa ^((~BCe)& BCi); Aba ^= gpu_KeccakF_RoundConstants[round+1];
...
static __device__ unsigned int keccak_absorb(uint64_t s[25], unsigned int pos,
    unsigned int r, const uint8_t *in, size_t inlen)
{
    unsigned int i;
    while (pos + inlen >= r) {
        for (i = pos; i < r; i++) s[i/8] ^= (uint64_t)*in++ << 8*(i%8);
        inlen -= r - pos; KeccakF1600_StatePermute(s); pos = 0;
    }
    for (i = pos; i < pos + (unsigned int)inlen; i++) s[i/8] ^= (uint64_t)*in++ << 8*(i%8);
    return i;
}
...
static __device__ unsigned int keccak_squeeze(uint8_t *out, size_t outlen,
    uint64_t s[25], unsigned int pos, unsigned int r)
{
    unsigned int i;
    while (outlen) {
        if (pos == r) { KeccakF1600_StatePermute(s); pos = 0; }
        for (i = pos; i < r && i < pos + (unsigned int)outlen; i++) *out++ = (uint8_t)(s[i/8] >> 8*(i%8));
        outlen -= i - pos; pos = i;
    }
    return pos;
}

static __device__ void keccak_absorb_once(uint64_t s[25], unsigned int r,
    const uint8_t *in, size_t inlen, uint8_t p)
{
    unsigned int i;
    for (i = 0; i < 25; ++i) s[i] = 0;
    while (inlen >= r) {
        for (i = 0; i < r / 8; ++i) s[i] ^= gpu_load64(in + 8 * i);
        KeccakF1600_StatePermute(s); inlen -= r; in += r;
    }
...
static __device__ void keccak_squeezeblocks(uint8_t *out, size_t nblocks, uint64_t s[25], unsigned int r)
{
    ...
    while (nblocks > 0) {
        ...
        for (i = 0; i < (r >> 3); i++) gpu_store64(out + 8 * i, s[i]);
        out += r; nblocks--;
    }
}
...
{ keccak_absorb_once(state->s, SHAKE256_RATE, input, inlen, 0x1F); state->pos = SHAKE256_RATE; }
...
static __device__ __noinline__ void shake128(uint8_t *out, size_t outlen, const uint8_t *in, size_t inlen)
{
    ...
    outlen -= nblocks * SHAKE128_RATE; out += nblocks * SHAKE128_RATE;
...
static __device__ __noinline__ void shake256(uint8_t *out, size_t outlen, const uint8_t *in, size_t inlen)
{
    ...
    outlen -= nblocks * SHAKE256_RATE; out += nblocks * SHAKE256_RATE;
...
static __device__ __noinline__ void sha3_256(uint8_t *output, const uint8_t *input, size_t inlen)
{
    ...
    KeccakF1600_StatePermute(s);
    for (size_t i = 0; i < 4; i++) gpu_store64(output + 8 * i, s[i]);
}
...
static __device__ __noinline__ void sha3_512(uint8_t *output, const uint8_t *input, size_t inlen)
{
    ...
    KeccakF1600_StatePermute(s);
    for (size_t i = 0; i < 8; i++) gpu_store64(output + 8 * i, s[i]);
}
